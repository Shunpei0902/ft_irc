...
/*   Updated: 2025/08/04 16:01:10 by sasano           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "command.hpp"

static bool check_modeformat(const std::string &mode_str)
{
    if (mode_str.empty() || (mode_str[0] != '+' && mode_str[0] != '-'))
        return false; // モード文字列が空または先頭が '+' または '-' でない場合は不正

    for (size_t i = 1; i < mode_str.size(); ++i)
    {
        if (mode_str[i] != 'i' && mode_str[i] != 'o' && mode_str[i] != 'k' &&
            mode_str[i] != 'l' && mode_str[i] != 't')
            return false; // モード文字列に不正な文字が含まれている場合は不正
    }
    return true; // 正常なモード文字列
}
